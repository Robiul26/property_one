...
class ListProperty extends BaseBlock
{
    ...
    {
        $model_property = Property::select("bravo_properties.*")->with(['location','translations','hasWishList', 'user', 'Category']);
        ...
        if (!empty($model['location_id'])) {
            ...
            if(!empty($location)){
                $model_property->join('bravo_locations', function ($join) use ($location) {
                    $join->on('bravo_locations.id', '=', 'bravo_properties.location_id')
        ...
        $model_property->orderBy("bravo_properties.".$model['order'], $model['order_by']);
        $model_property->where("bravo_properties.status", "publish");
        $model_property->with('location');
        $model_property->groupBy("bravo_properties.id");
